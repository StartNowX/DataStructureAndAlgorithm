...
class Solution {
   public:
    double new21Game(int N, int K, int W) {
         ...
         *  即：f(x) = 1 / W * (f(x + 1 + W) - f(x + 1))，其中，0 <= x < K-1
         *
         *  而对于x=k-1的情况，f(k-1) = (f(k) + f(k+1) + f(k+2) + ... + f(k+w-1)) / W，
         *  而只有当K<x<min(K+W-1,N)时f(x)才为1，因此简化f(k-1)为：
         *     f(x-1) = min(N, K+W-1) - K + 1 / W = min(N - K + 1, W) / W
         */
